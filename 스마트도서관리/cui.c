...
void new_title() {
	...
	gotoxy(x, y + 12); printf("♤ 도서 정보	: 등록된 도서의 모든 정보를 출력합니다.");
	gotoxy(x, y + 14); printf("♤ 도서 검색	: 등록된 도서 중 검색한 도서 정보를 출력합니다.");
	gotoxy(x, y + 16); printf("♤ 도서 대여	: 등록된 도서 중 원하는 도서를 대여합니다.");
	gotoxy(x, y + 18); printf("♤ 도서 반납	: 대여한 도서를 반납 및 대여 연장합니다.");
	gotoxy(x, y + 20); printf("♤ 도서 추천	: 검색횟수로 (인기,분류,저자) 도서를 추천합니다.");
	gotoxy(x, y + 22); printf("♤ 도서 구매	: 도서 구매 정보를 출력합니다.");
	gotoxy(x, y + 24); printf("♤ 도서 관리	: 관리자 번호를 입력해야만 사용할 수 있습니다.");
	gotoxy(x, y + 28); printf("      ▶▶▶▶메인메뉴 이동은 아무키나 누르세요!◀◀◀◀");
	gotoxy(x, y + 32); printf("♧ 1조, 개발기간 : 2022년 10월17일~10월 27일");
	gotoxy(x, y + 34); printf("♧ 전병조,김종욱,김성혜,【박수인】,전병주(발표일:10월28일)");
