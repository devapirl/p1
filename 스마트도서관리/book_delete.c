...
void delete_file()
{
	...
	case 1:
	{
		...
		while (fread(&dt_t, sizeof(dt_t), 1, fpdelete) == 1)
		{
			if (strcmp(deletename, dt_t.book.name) == 0 && delete_book_sn == dt_t.book.sn)
			{
				found = 1;
		...
		if (strcmp(edit_search_book, ed.book.name) == 0 && edit_book_sn == ed.book.sn)
		{
			...
			case 1:
				...
				if (dest[idx].book.loan == 1)	// 대여자 정보가 바뀌었을때만 대여자 구조체 정보 수정?
				{
					...
					system("cls");
					char a[40] = { 0 };
					dest[idx].user.delay = get_time_3day_add(a);
					edit_printf_all(dest[idx]);
				...
				{
					printf(" 수정 중인 도서가 대여 중인 상태가 아닙니다.\n");
					printf(" 해당 도서의 대여 정보를 초기화합니다.\n");
				...
				edit_printf_all(dest[idx]);
				printf(" 새로 바꿀 도서 구매 사이트를 입력하세요.\n");
				scanf("%s", dest[idx].buybook.site);
				system("cls");
				edit_printf_all(dest[idx]);
				printf(" 새로 바꿀 도서 구매 가격을 입력하세요.\n");
				...
				{
					printf(" 수정 중인 도서의 영화가 없습니다.\n");
					printf(" 해당 도서의 영화 정보를 초기화합니다.\n");
